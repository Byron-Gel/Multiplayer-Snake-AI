...
class Snake:
    ...
    def __init__(self, pos, length=4, w=W, h=H):
        """
        ...
        self.START_POS = [(self.w / 4, self.h / 4), (self.w / 4, self.h * 3 / 4), (3 * self.w / 4, self.h / 4),
                          (3 * self.w / 4, self.h * 3 / 4)]
        self.pos = pos
    ...
    def play_step(self, actions):
        ...
        for i in range(len(self.snakes)):
            if self.snakes[i].is_collision(i, self.snakes, self.frame_iteration):
                ...
                # self.num_snakes -= 1
                snake.reward -= DEATH_MULT
                snake = self.snakes[i]
                # snake.pos = snake.pos % 4
        ...
        game_over = False
        for snake in self.snakes:
            if snake.apples >= 5 or self.frame_iteration > 100:
                game_over = True
                break
